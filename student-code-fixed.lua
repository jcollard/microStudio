...
function check_collisions()
  ...
  for jx, laser in pairs(lasers) do
    ...
    for ex, enemy in pairs(enemies) do
      -- Only check if enemy is not already destroyed
      if not enemy.isDestroyed then
        if boxes_colliding(laser, enemy) then
          enemy.isDestroyed = true
          -- Move laser off screen so it can be reused
          laser.y = 300
        end
      end
    end
...
function update_enemies()
  for ix, enemy in pairs(enemies) do
    if not enemy.isDestroyed then
      ...
      if enemy.x < -100 then enemy.vx = math.abs(enemy.vx) end
      if enemy.x > 100 then enemy.vx = -math.abs(enemy.vx) end
      if enemy.y < -100 then enemy.vy = math.abs(enemy.vy) end
      if enemy.y > 100 then enemy.vy = -math.abs(enemy.vy) end
    end
  end
end
