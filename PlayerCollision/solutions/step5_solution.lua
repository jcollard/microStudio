-- ============================================
-- SOLUTION FOR STEP 5: ADDING A LIFE COUNT
-- ============================================
-- Added lives variable, player_hit(), updated draw_ui() with lives display
...
-- ===== STEP 5: UPDATED check_collisions() =====
  ...
  for ix, enemy in pairs(enemies) do
    if not enemy.isDestroyed then
      if boxes_colliding(player, enemy) then
        player_hit()  -- NEW: Use player_hit() instead of direct assignment
        enemy.isDestroyed = true
        return
      end
    end
